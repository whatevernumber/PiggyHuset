...
<article class:rounded class:unavailable={article?.status_id === 5 || article?.status_id === 6 ? 'unavailable' : ''} id="{article.id}" data-sveltekit-preload-data="{type === 'article' ? 'hover' : 'tap'}">
    {#if type === 'info'}
